...
def merge_t_tables(cfg):
    ...
    for table in cfg.t_tables.tables:
        ...
        print([table.include, str(cfg.t_tables.link)])
        t_table = t_table.join(t_adm.select([table.include, str(cfg.t_tables.link)]), left_on=table.table_link, right_on=cfg.t_tables.link)

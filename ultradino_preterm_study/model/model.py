...
class PretermModel(nn.Module):
    ...
    def from_conf(cls, cfg):
        ...
        if cfg.encoder.weights_path:
            ...
            encoder = load_pretrained_ultradino(
                cfg.encoder.type,
            ...
            encoder = load_from_scratch(cfg.encoder.type)
        ...
        if cfg.decoder.type == 'spacing':
            if cfg.landmarks:
                decoder = SpacingDecoder(encoder.embed_dim + 64, cfg.pixel_spacing_embed_dim, cfg.decoder.outputs)
            else:
                decoder = SpacingDecoder(encoder.embed_dim, cfg.pixel_spacing_embed_dim, cfg.decoder.outputs)
            
        elif cfg.decoder.type == 'basic':
            decoder = BasicDecoder(encoder.embed_dim, cfg.pixel_spacing_embed_dim, cfg.decoder.outputs)
#BasicDecoder(num_inputs, cfg.decoder.outputs)
    ...
    def forward(self, img, pxs):
        
        plane_embedding = self.encoder(img)
        ...
        return self.decoder.output_to_dict(dec_out)
...
class PretermFinetuning(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        if self.model.has_pixel_spacing_encoder():
            param_groups.append(
                {"params": self.model.pixel_spacing_encoder.parameters(), "lr": self.learning_rate}
            )
        if self.model.has_landmarks_encoder():
            param_groups.append(
                {"params": self.model.landmarks_encoder.parameters(), "lr": self.learning_rate}
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        logit = outputs['preterm'].squeeze(-1)
        ...
        if self.use_proxy:
             pred_CL = outputs['CL']
             CL = inputs['CL']
             loss = self.classification_loss(logit, label) + self.weightedRE(pred_CL, CL)#, torch.Tensor([0.3]))
        else:
    ...
    def validation_step(self, batch, batch_idx):
        ...
        logs = {}
        val_loss = self.classification_loss(logit, label)
        logs["loss"] = val_loss

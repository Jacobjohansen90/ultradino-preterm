...
def test_model(folder_path, testdata_path, move=True):
    ...
    for i, weights in enumerate(dirs):
        ...
        with torch.no_grad():
            ...
            wr_all.writerow([SensSpec_avg, SensSpec_max, Sens_avg, Spec_avg, Sens_max, Spec_max, cutoff_avg, cutoff_max, weights])
    ...
    with open(folder_path + 'test_top_5.csv', 'w') as f:
        wr = csv.writer(f)
        wr.writerow(['Sens@Spec', 'type', 'Sens', 'Spec', 'weights'])

        for i in range(5):
            idx, col, _ = top_5.row(i)
            row = df[idx]
            t = col.split('_')[1]
            wr.writerow([row[col], t, row['Sens_'+t], row['Spec_'+t], row['weights']])
